      ...
      <div className="card-soft flex flex-col h-[70vh] overflow-hidden">
        <div ref={ref} className="flex-1 overflow-y-auto p-6 space-y-4" data-testid="chat-thread">
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
              {m.role === "assistant" && (
                <div className="w-9 h-9 rounded-xl bg-[#FF5E3A] grid place-items-center text-white shrink-0"><Sparkles className="w-4 h-4" /></div>
              )}
          ...
          {loading && (
            <div className="flex gap-3">
              <div className="w-9 h-9 rounded-xl bg-[#FF5E3A] grid place-items-center text-white"><Loader2 className="w-4 h-4 animate-spin" /></div>
        ...
        {messages.length <= 1 && (
          <div className="px-6 pb-3 flex flex-wrap gap-2">
            {SUGGESTED.map((s) => (
              <button key={s} onClick={() => send(s)} className="text-xs px-3 py-1.5 rounded-full border border-black/10 hover:border-[#FF5E3A] hover:text-[#FF5E3A] bg-white" data-testid={`suggested-${s.slice(0,10)}`}>{s}</button>
            ))}
          </div>
        )}
        ...
        <div className="border-t border-black/5 p-4 flex items-center gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-full bg-black/[0.04] outline-none text-sm focus:ring-2 focus:ring-[#FF5E3A]/30"
            data-testid="chat-input"
          />
          <button onClick={() => send()} disabled={loading} className="w-11 h-11 rounded-full bg-[#FF5E3A] grid place-items-center text-white hover:bg-[#E04B28] disabled:opacity-60" aria-label="send" data-testid="chat-send"><Send className="w-4 h-4" /></button>
        </div>
      </div>
    </div>
  );

...
import { CalendarDays, Download, GripVertical, RefreshCw, Save, Users } from "lucide-react";
...
export default function Timetable() {
  ...
  const save = async () => {
    ...
    if (!table) return;
    await api.put(`/timetable/${classId}`, { entries: table.entries });
    ...
    <div className="space-y-6" data-testid="timetable-page">
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">Generate conflict-free weekly timetables, then drag, edit, save, and print.</p>
        </div>
      ...
      <div className="card-soft p-5 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2 text-xs text-[#4A7C59] font-medium"><Users className="w-4 h-4" /> Conflict checks enabled</div>
      </div>
              ...
              {days.map((day) => {
                ...
                return (
                  <div key={`${day}-${period}`} onDragOver={(e) => e.preventDefault()} onDrop={(e) => moveEntry(e.dataTransfer.getData("entry"), entry?.id)} className="p-2 border-t border-l border-black/5 min-h-24">
                    {entry ? (
                      <div draggable={entry.type !== "break"} onDragStart={(e) => e.dataTransfer.setData("entry", entry.id)} className={`h-full rounded-xl p-3 ${entry.type === "break" ? "bg-neutral-100 text-neutral-500" : entry.type === "lab" ? "bg-[#E5EFE8] text-[#2F5D3A]" : "bg-[#FBE9E3] text-[#0A1128]"}`}>

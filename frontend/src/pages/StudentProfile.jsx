      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card-soft p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <Sparkles className="w-5 h-5 text-[#E05236]" />
          </div>
          <div className="h-72">
            <ResponsiveContainer>
              <RadarChart data={radar} outerRadius="75%">
                ...
                <Radar name={s.name} dataKey="score" stroke="#E05236" fill="#E05236" fillOpacity={0.25} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {(s.fees || []).map((f) => (
              <div key={f.id} className="flex items-center justify-between rounded-lg border border-black/5 px-3 py-2.5">
                ...
                <div className="text-right">
                  ...
                  <div className={`text-xs font-medium ${f.status === "paid" ? "text-[#4A7C59]" : "text-[#E05236]"}`}>{f.status}</div>
                </div>
              </div>
            ))}

  ...
  return (
    <div className="space-y-6" data-testid="ai-teacher-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        {TASKS.map((t) => (
          <button key={t.id} onClick={() => setTask(t.id)} className={`text-left card-soft p-5 transition ${task === t.id ? "ring-2 ring-[#FF5E3A] !bg-[#FFF3F0]" : "hover:-translate-y-0.5"}`} data-testid={`task-${t.id}`}>
            <t.icon className={`w-6 h-6 ${task === t.id ? "text-[#FF5E3A]" : "text-[#0A1128]"}`} strokeWidth={1.5} />
      ...
      <div className="card-soft p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            ...
            <input value={subject} onChange={(e) => setSubject(e.target.value)} className="mt-1 w-full px-4 py-3 rounded-xl border border-black/10 bg-white outline-none focus:ring-2 focus:ring-[#FF5E3A]/30 focus:border-[#FF5E3A]" data-testid="ai-subject" />
          </div>
          <div>
            <label className="text-xs font-medium text-neutral-600">Grade</label>
            <input value={grade} onChange={(e) => setGrade(e.target.value)} className="mt-1 w-full px-4 py-3 rounded-xl border border-black/10 bg-white outline-none focus:ring-2 focus:ring-[#FF5E3A]/30 focus:border-[#FF5E3A]" data-testid="ai-grade" />
          </div>
          <div>
            <label className="text-xs font-medium text-neutral-600">Topic</label>
            <input value={topic} onChange={(e) => setTopic(e.target.value)} className="mt-1 w-full px-4 py-3 rounded-xl border border-black/10 bg-white outline-none focus:ring-2 focus:ring-[#FF5E3A]/30 focus:border-[#FF5E3A]" data-testid="ai-topic" />
          </div>
        </div>
        <textarea value={extra} onChange={(e) => setExtra(e.target.value)} rows={2} placeholder="Any extra context (optional)" className="mt-3 w-full px-4 py-3 rounded-xl border border-black/10 bg-white outline-none resize-none focus:ring-2 focus:ring-[#FF5E3A]/30" data-testid="ai-extra" />
      ...
      <div className="card-soft p-6 min-h-[280px]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2"><Bot className="w-5 h-5 text-[#FF5E3A]" /><div className="font-display text-lg font-semibold">Output</div></div>

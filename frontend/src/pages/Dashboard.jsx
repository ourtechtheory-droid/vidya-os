  ...
  return (
    <div className="space-y-6" data-testid="dashboard-home">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPI label="Today present rate" value={`${todayAttendance.present_rate ?? 0}%`} sub={todayAttendance.date ? new Date(todayAttendance.date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" }) : "Today"} icon={Calendar} accent="bg-[#FBE9E3] text-[#E05236]" />
        <KPI label="Merit percentage" value={`${stats?.merit_percentage ?? 0}%`} sub="Click for exam stats" icon={TrendingUp} accent="bg-[#E5EFE8] text-[#4A7C59]" onClick={() => setShowMerit((v) => !v)} />
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={meritBreakdown}>
                ...
                <Bar dataKey="count" fill="#4A7C59" radius={[8, 8, 0, 0]} barSize={34} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {canUseCalendar && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" data-testid="teacher-calendar">
          <div className="card-soft p-6 lg:col-span-2">
            ...
            <div className="mt-2 grid grid-cols-7 gap-2">
              {calendarCells.map((day, index) => {
                ...
                return (
                  <button key={`${day || "blank"}-${index}`} type="button" disabled={!day} onClick={() => pickCalendarDate(day)} className={`aspect-square rounded-lg border text-sm relative ${active ? "border-[#E05236] bg-[#FBE9E3] text-[#E05236] font-semibold" : "border-black/5 bg-white hover:border-black/20"} disabled:bg-transparent disabled:border-transparent`}>
                    {day}
                    {eventDates.has(iso) && <span className="absolute bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#E05236]" />}
                  </button>
                );
              })}
            </div>
          ...
          <form onSubmit={createEvent} className="card-soft p-6 space-y-3">
            <div className="flex items-center gap-2"><BookOpen className="w-5 h-5 text-[#E05236]" /><div className="label-eyebrow">Teacher calendar</div></div>
            ...
            <div className="mt-4 divide-y divide-black/5">
              ...
              {events.map((event) => (
                <div key={event.id} className="py-3 flex items-start gap-3">
                  <div className="w-14 shrink-0 rounded-lg bg-[#FBE9E3] text-[#E05236] px-2 py-2 text-center">
                  ...
                  <button onClick={() => deleteEvent(event.id)} className="p-2 rounded-lg text-neutral-400 hover:text-[#E05236] hover:bg-[#FBE9E3]" aria-label={`delete ${event.title}`}><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </div>
